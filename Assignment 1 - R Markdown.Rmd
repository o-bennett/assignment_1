...
I was also able to perform visualisation techniques to view characteristics such as the distribution of reviews.
...
I also modified certain variables so that they could be used in the predictive models. Using the 'lubridate' package on the string type `date` variable, I created factor variables for the year, month and day of the week that the reviews were written. From the `yelping_since` variable, I also calculated the number of days between the account being created and the review written and deleted observations with a negative number of days, as this would obviously be due to a error. Finally, I converted `state` to a factor variable. After these changes I was left with a `clean_data` dataframe that contained 31 unique variables, as seen below:
...
| `compliment_funny` | Numeric (integer) | Number of funny compliments received by the user |
...
I regularly revisited and refined the data preparation stage, making adjustments to the dataframe as I gained new insights in subsequent stages. This included re-adding variables that had previously been removed. I originally removed variables that contained more than 50% entries with the value 0, however, I later added these variables back into the dataframe, which resulted in more accurate predictions. I also removed some observations after noticing that the coefficients on state variables differed greatly and subsequently discovering that seven of the states only contained one or two reviews.
...
Finally, while examining the `numeric_df`, I found that the variable `review_day_of_week` was automatically encoded using a polynomial contrast, whereas `review_month` and `review_year` defaulted to treatment (dummy) contrasts. After experimenting with various combinations, I discovered that the optimal approach for maximizing predictive accuracy involved using polynomial contrasts for both `review_day_of_week` and `review_month`, while retaining a treatment contrast for `review_year`. 
...
| Ridge | 1.180249 | 1.188901 |
| LASSO | 1.177757 | 1.183692 |
| Regression tree | 1.313129 | 1.313515 |
|Randomly generated| 4.723367 | 4.6496 |
| Rounded Ridge | 1.260562 | 1.2726 |
| Rounded Lasso | 1.258968 | 1.2666 |
...
| Rounded Ridge | 0.385501 | 0.5523 |
| Rounded Lasso | 0.3895882 |  |
...
- sentiment analysis could be very powerful in prediction given the context
- given I had more computing power, ADA boost + random forest
